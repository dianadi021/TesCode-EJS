...
const GetStatusData = async (req, res) => {
  try {
    ...
    if (isHasSyntax && nama_status) {
      ...
      const isDocumentHasInDatabase = await StatusModel.aggregate([
        { $project: { _id: 1, nama_status: 1, harga: 1 } },
        { $match: toFilter },
      ]);
    ...
    const isDocumentHasInDatabase =
      !page && !document
        ? await StatusModel.aggregate([{ $project: { _id: 1, nama_status: 1, harga: 1 } }])
